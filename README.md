...
一条命令生成完整的演示数据，包含管线运行结果、模拟人工纠偏判决、自进化学习记录，即可启动 Web UI 展示全部功能。

```bash
make demo          # 生成演示数据（离线管线 + 人工判决注入 + 学习提取）
make demo-reset    # 清空数据后重新生成
make web           # 启动 Web 服务 → http://localhost:8000
```

演示数据基于 `docs/` 目录下的 2015 版 vs 2026 版天猫服务协议，包含 ~400 条差异、30 条模拟人工判决（确认/纠偏/驳回），覆盖自进化记录、风险趋势、三态纠偏、PDF 导出全部页面。

| 页面 | 演示内容 |
|------|---------|
| `/` | 上传页，可直接开始新的比对 |
| `/job/{id}` | SSE 实时进度（demo 已完成的 job 会直接跳转结果页） |
| `/results/{id}` | ~400 条差异 + 30 条人工判决，可筛选/纠偏/导出 PDF |
| `/learnings` | 累计运行统计、风险趋势、高频纠偏类别 |

> **可重复演示**：`make demo-reset && make demo && make web` 每次生成一致的展示数据（固定随机种子，确保团队演示 / 客户 demo 效果一致）。
...
每轮比对完成后自动提取：风险模式分布、LLM 校验拒绝率、高置信度模式。**V2 新增人工纠偏反馈闭环**：用户在 Web UI 对每条 LLM 判断做「确认/驳回/纠偏」三态标记 → 持久化到服务端 → 下次比对时自动注入为"人工纠偏经验"（"过往 5 次中，R01 交付时效被人工纠正 3 次——请优先复核此类判断"）。形成 **运行越多越准确** 的正循环。
...
```json
{
  ...
  "human_verdict": {
    "action": "confirmed",
    "timestamp": "2026-05-24T03:13:45Z",
    "original": {"risk_level": "high", "risk_categories": ["R01"]}
  }
}
```
...
| `/results/{id}` | 结果页：风险筛选、类型筛选、**三态人工纠偏**（确认/驳回/纠偏 → 服务端持久化）、备注自动保存、右侧悬浮确认导航、PDF 导出 |
...
```bash
make test   # 122 个测试，覆盖：提取/解析/对齐/传统Diff/L2校验/自进化/Web路由
```
